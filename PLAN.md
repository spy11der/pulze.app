...
- **Crew tab** — Friends check-in activity feed. Vertical list with small square stamped photo thumbnails, friend name, venue, neighborhood, and time ago. Proximity hint at the top when a friend is nearby. Tapping a check-in opens the check-in detail screen.
...
- **Check-in detail** — Tapping a friend's check-in in the Crew feed opens this screen. Large hero photo of the friend's check-in selfie with their name and caption overlaid at the bottom. Below the photo: venue name with map pin, a simple busyness indicator (status and percentage in white), and time ago. Personal and minimal — no venue stats, vibe, or photos.
