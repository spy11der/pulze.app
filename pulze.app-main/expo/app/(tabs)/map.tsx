...
import { fetchWalkingRoute, type WalkingRouteResult } from '@/services/googleRoutes';
...
interface ActiveWalkingRoute extends WalkingRouteResult {
  destination: Pick<MapVenue, 'id' | 'name' | 'address' | 'latitude' | 'longitude'>;
  origin: Coordinates;
}
...
function getDirectionsUrl(destination: MapVenue) {
  if (Platform.OS === 'ios') {
    return `http://maps.apple.com/?daddr=${destination.latitude},${destination.longitude}&dirflg=w`;
  }
  return `https://www.google.com/maps/dir/?api=1&destination=${destination.latitude},${destination.longitude}&travelmode=walking`;
}

function getDistanceBetweenCoordinates(a: Coordinates, b: Coordinates): number {
...
function formatWalkingDistance(distanceMeters: number): string {
  ...
  if (miles < 0.2) {
    const feet = Math.max(50, Math.round((distanceMeters * 3.28084) / 10) * 10);
    return `${feet} ft`;
  }
  return `${miles.toFixed(miles >= 10 ? 0 : 1)} mi`;
}

function hasGoogleRoutesApiKey(): boolean {
...
function NativeMapContent({
  ...
  activeRoute: ActiveWalkingRoute | null;
}) {
  ...
  const routeCoordinates = useMemo(() => {
    ...
    const baseCoordinates = activeRoute.polylineCoordinates.length >= 2
      ? activeRoute.polylineCoordinates
      : fallbackCoordinates;

    return baseCoordinates.filter(
      (coordinate, index, coordinates) => Number.isFinite(coordinate.latitude)
  ...
  return (
    <View
      style={[
        ...
        {
          top: topOffset ?? 12,
          backgroundColor: isDark ? 'rgba(4, 19, 24, 0.92)' : 'rgba(255,255,255,0.95)',
    ...
    >
      <View style={styles.routeBannerContent}>
        <View style={[styles.routeIconWrap, { backgroundColor: colors.aquaBright }]}> 
          {isLoading ? (
        ...
        <View style={styles.routeBannerTextWrap}>
          <Text style={[styles.routeBannerTitle, { color: colors.text }]} numberOfLines={1}>
            Walk to {route.destination.name}
          </Text>
          <Text style={[styles.routeBannerSubtitle, { color: colors.textMuted }]} numberOfLines={1}>
            {formatWalkingDuration(route.durationSeconds)} · {formatWalkingDistance(route.distanceMeters)}
            {isLoading ? ' · updating route…' : ''}
          </Text>
        </View>
        <Pressable
          onPress={onStop}
          style={({ pressed }) => [
            styles.routeStopButton,
            {
          ...
          testID="route-stop-button"
        >
          <Text style={[styles.routeStopText, { color: colors.text }]}>Stop</Text>
        </Pressable>
      </View>
    </View>
  );
...
export default function MapScreen() {
  ...
  const lastRerouteRef = useRef<{ origin: Coordinates; timestamp: number } | null>(null);
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [userLocation, setUserLocation] = useState<Coordinates | null>(null);
  const [isLocating, setIsLocating] = useState<boolean>(false);
  const [isRouting, setIsRouting] = useState<boolean>(false);
  const [activeRoute, setActiveRoute] = useState<ActiveWalkingRoute | null>(null);
  ...
  const fitRouteOnMap = useCallback((origin: Coordinates, destination: Coordinates, routePoints: Coordinates[]) => {
    if (Platform.OS === 'web' || !mapRef.current) return;

    const coordinates = (routePoints.length >= 2 ? routePoints : [origin, destination]).filter(
      (coordinate) => Number.isFinite(coordinate.latitude) && Number.isFinite(coordinate.longitude)
    );

    if (coordinates.length < 2) {
      console.log('[MapScreen] Skipping fitToCoordinates because route points were invalid');
      return;
    }
    ...
    try {
      ...
      console.log('[MapScreen] Failed to fit route on map', error);
    }
  }, []);
  ...
  const locateUser = useCallback(async (): Promise<Coordinates | null> => {
    try {
      setIsLocating(true);
  ...
  const requestWalkingRoute = useCallback(async (
    destination: Pick<MapVenue, 'id' | 'name' | 'address' | 'latitude' | 'longitude'>,
    options?: { origin?: Coordinates; silent?: boolean; fallbackToExternal?: boolean }
  ) => {
    const origin = options?.origin ?? userLocation ?? await locateUser();
    ...
    if (!hasGoogleRoutesApiKey()) {
      Alert.alert(
        'Google key missing',
        'Add EXPO_PUBLIC_GOOGLE_MAPS_API_KEY to expo/.env, restart the Expo server, and try again.'
      );
      if (options?.fallbackToExternal && selectedVenue) {
    ...
    try {
      ...
      const nextRoute: ActiveWalkingRoute = {
        ...route,
        destination,
        origin,
      };
      ...
      fitRouteOnMap(origin, destination, route.polylineCoordinates);
      lastRerouteRef.current = { origin, timestamp: Date.now() };
      return nextRoute;
    } catch (error) {
      console.log('[MapScreen] Failed to fetch walking route', error);
      if (!options?.silent) {
        const message = error instanceof Error ? error.message : 'We could not load walking directions right now.';
        Alert.alert('Directions unavailable', message);
      }
      return null;
    } finally {
      setIsRouting(false);
    }
  }, [fitRouteOnMap, locateUser, openUrl, selectedVenue, userLocation]);

  const handleDirections = useCallback(async () => {
    if (!selectedVenue) return;
    await requestWalkingRoute(selectedVenue, { fallbackToExternal: true });
  }, [requestWalkingRoute, selectedVenue]);
  ...
  const shouldShowVenueSheet = Boolean(selectedVenue) && !isFullscreen && !activeRoute && !isRouting;
  const shouldShowFullscreenVenueSheet = Boolean(selectedVenue) && !activeRoute && !isRouting;
  ...
  useEffect(() => {
    if (!activeRoute || Platform.OS === 'web') {
      stopRouteTracking();
      return;
    }
      ...
      try {
        ...
        routeWatcherRef.current = await Location.watchPositionAsync(
          ...
          (result) => {
            ...
            const lastReroute = lastRerouteRef.current;
            const movedEnough = !lastReroute || getDistanceBetweenCoordinates(coords, lastReroute.origin) >= 30;
            const waitedLongEnough = !lastReroute || (Date.now() - lastReroute.timestamp) >= 12000;

            if (!movedEnough || !waitedLongEnough) return;

            void requestWalkingRoute(destination, { origin: coords, silent: true });
          }
        );
      } catch (error) {
        console.log('[MapScreen] Failed to watch user location', error);
      }
    };
  ...
  }, [activeRoute, requestWalkingRoute, stopRouteTracking]);
    ...
    <View style={[styles.screen, { backgroundColor: colors.background }]} testID="map-screen">
      ...
      >
        ...
        <View style={[styles.mapContainer, { borderColor: colors.border }]}>
          ...
          />
          {activeRoute ? (
            <RouteBanner route={activeRoute} isLoading={isRouting} onStop={stopActiveRoute} />
          ) : null}
          {legendBar}
          {shouldShowVenueSheet && selectedVenue ? (
            <BottomSheet
              venue={selectedVenue}
              onClose={closeSheet}
      ...
      <Modal visible={isFullscreen} animationType="slide" statusBarTranslucent testID="fullscreen-map-modal">
        <View style={[styles.fullscreenContainer, { backgroundColor: colors.background }]}>
          ...
          {shouldShowFullscreenVenueSheet && selectedVenue ? (
            <BottomSheet
              venue={selectedVenue}
              onClose={closeSheet}
              onDirections={() => void handleDirections()}
...
const styles = StyleSheet.create({
  ...
  routeBannerTextWrap: { flex: 1, gap: 2 },
  routeBannerTitle: { fontSize: 13, fontWeight: '800' as const },
  routeBannerSubtitle: { fontSize: 11, fontWeight: '600' as const },
  routeStopButton: {
  ...
  routeStopText: { fontSize: 12, fontWeight: '700' as const },
