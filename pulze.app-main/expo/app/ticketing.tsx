...
export default function TicketingScreen() {
  ...
  const { event, isError, error, isFetching, isEventbrite } = useTicketEvent();
  const eventbriteConfigured = hasEventbriteConfig();
  ...
  useEffect(() => {
    setQuantities(prev => Object.fromEntries(
      Object.entries(prev).filter(([tierId]) => event.ticketTiers.some(tier => tier.id === tierId))
    ));

    if (selectedTier && !event.ticketTiers.some(tier => tier.id === selectedTier)) {
      setSelectedTier(null);
    }
  }, [event.ticketTiers, selectedTier]);
  ...
  const lowestAvailablePrice = useMemo(() => {
    const availablePrices = event.ticketTiers.filter(tier => !tier.soldOut).map(tier => tier.price);
    return availablePrices.length ? Math.min(...availablePrices) : 0;
  }, [event.ticketTiers]);

  const ticketingStatus = useMemo(() => {
    if (isEventbrite) {
      return 'Live Eventbrite event loaded. Checkout will continue on Eventbrite.';
    }

    if (eventbriteConfigured && isError) {
      return error instanceof Error
        ? `Eventbrite fetch failed (${error.message}). Showing fallback sample data.`
        : 'Eventbrite fetch failed. Showing fallback sample data.';
    }

    if (!eventbriteConfigured) {
      return 'Add EXPO_PUBLIC_EVENTBRITE_API_KEY and EXPO_PUBLIC_EVENTBRITE_EVENT_ID in expo/.env to load a live Eventbrite event.';
    }

    return 'Loading Eventbrite event...';
  }, [error, eventbriteConfigured, isError, isEventbrite]);
          ...
          <View style={styles.mainContent}>
            ...
            <View style={[styles.integrationNotice, { backgroundColor: isDark ? 'rgba(53,212,207,0.08)' : 'rgba(26,168,163,0.06)', borderColor: colors.border }]}> 
              <View style={styles.integrationNoticeRow}>
                ...
                <Text style={[styles.integrationNoticeText, { color: colors.textMuted }]}>{ticketingStatus}</Text>
              </View>
      ...
      <View style={[styles.stickyBottom, { paddingBottom: insets.bottom + 8, backgroundColor: isDark ? 'rgba(4,19,24,0.95)' : 'rgba(245,248,250,0.95)', borderTopColor: colors.border }]}>
        <View style={styles.stickyInfo}>
          {selectedTierData ? (
            ...
            <>
              <Text style={[styles.stickyPrice, { color: colors.text }]}>From ${lowestAvailablePrice}</Text>
